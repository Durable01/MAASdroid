...
def find_node_in_bounds(root, nearest_bound, bounds_data):
    ...
    # Iterate through bounds_data
    for bounds_set in bounds_data:
        # Find the node with bounds equal to bounds_set
        target_node = None
        for node in root.findall(".//node"):
            node_bounds_str = node.get('bounds')
            if node_bounds_str and parse_bounds(node_bounds_str) == bounds_set:
                target_node = node
                break

        if target_node:
            # Find the first focusable leaf node within target_node
            child = target_node.find(".//node[@focusable='true']")
            if child is not None:
                node_bounds_str = child.get('bounds')
                node_bounds = parse_bounds(node_bounds_str)
                node_class = child.get('class')
                node_text = child.get('text') or child.get('content-desc')
                path.append((node_bounds, node_text, node_class))
...
def find_node_in_bounds_reversed(root, nearest_bound, bounds_data):
    path = []

    # Iterate through bounds_data
    for bounds_set in reversed(bounds_data):
        # Find the node with bounds equal to bounds_set
        target_node = None
        for node in root.findall(".//node"):
            node_bounds_str = node.get('bounds')
...
def get_path(xml_folder, input_file, input_bounds_file, output_file):
    ...
    for line in bounds_text.splitlines():
        if line.startswith('F:'):
            ...
            # bounds_data.append((xml_file, eval(bounds_str)))
